...
Typing requires no microphone or speech permission. Pressing the microphone button requests Microphone and Speech Recognition permissions for on-device English dictation. Denials show Settings instructions; no cloud fallback is used. Recording ends on Stop, command submission, window closure/backgrounding, completion, error, or a 30-second deadline. Review recognized text before sending. No wake-word or clap listener runs. Accessibility and notification permissions are not requested. Screen Recording is requested only after `Look at my screen` or `Capture dashboard`. Normal dashboard analysis reads Safari page text and does not use Screen Recording. Opening a supported file may trigger an ordinary macOS protected-folder access prompt; denying access must not be bypassed. App/URL opening uses NSWorkspace.
