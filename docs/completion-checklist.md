# ULTRON completion checklist

ULTRON is a working development app, not a finished general-purpose computer agent.

## Available

- Native Mac interface, original animated core, configurable native speech, provider-independent speech interface.
- Explicit dictation and bounded Hey Ultron sessions; live audio acceptance remains pending.
- Installed-app opening, Safari URL opening, restricted document/folder opening, dashboard module selection.
- Requested Safari page text extraction with origin checks and source-verified observation candidates. Screenshot capture remains a separate development vision path.
- Provider-independent AI interface, development mock, optional Apple on-device question answering, bounded ephemeral conversation memory, and Clear Conversation.
- Encrypted temporary iPhone-to-Mac sessions. Simulator-to-Mac Open Calculator was verified and the session revoked.
...
7. **Release quality:** complete permission-denial tests, accessibility and performance review, security review, signing/notarization, update distribution, and recovery diagnostics without private content.
...
Supported fixed commands still use deterministic routing. `What are we seeing?` continues to read the configured dashboard. General questions have no automatic web, screen, file, or action access. On-device AI requires macOS 26 and an available Apple Intelligence model; unavailable models produce an explicit error with no cloud fallback.
